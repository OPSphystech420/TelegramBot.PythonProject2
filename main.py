        ...
        if check[0] == True:
            await bot.send_message(chat_id=msg.from_user.id,
                                    text=f'<b>Добро пожаловать!\n\n</b>'
                                          f'<b>Я @END_RAID - моя цель сделать игру между двумя игроками самой запоминающейся!\n</b>',
                                    reply_markup=menu.main_menu())
            await bot.send_message(config.config('admin_group'), f'Новый пользователь {msg.from_user.get_mention(as_html=True)}')
            if check[1] != 0:
                func.check_in_bd(User(check[1]).update_balance(config.config('ref_reward')))
                await bot.send_message(chat_id=check[1], 
                                text=f'За приглашение {User(msg.from_user.id).first_name} вам начислено {config.config("ref_reward")} ₽')
                await bot.send_message(chat_id= config.config("admin_group"),
                	               text = f'{User(check[1]).first_name} пригласил @{User(msg.from_user.id).username} {msg.from_user.id}')
        else:
            if User(msg.from_user.id).ban == 'no':
                ...
                if some_var.status == 'member' or some_var.status == 'administrator' or some_var.status == 'creator':
                    await bot.send_message(chat_id=msg.from_user.id,
                                        text=f'{msg.from_user.first_name} рад видеть тебя снова!', reply_markup = menu.main_menu())
                else:
...
async def handler_call(call: types.CallbackQuery, state: FSMContext):
    ...
    if User(chat_id).ban == 'no':
        ...
        if call.data == 'create_promo':
            ...
            await call.message.answer('Введи название промокода, который хочешь создать, для отмены введи "-", без ковычек')
...
async def withdraw_sum(message: types.Message, state: FSMContext):
    ...
    try:
        ...
        if qiwi[:1] == '7' and len(qiwi) == 11 or qiwi[:3] == '380' and len(qiwi[3:]) == 9 or qiwi[:3] == '375' and len(qiwi) <= 12:
            if qiwi.isdigit() == True:
                async with state.proxy() as data:
                    data['qiwi'] = qiwi
                await Withdraw.next()
        ...
        await state.finish()
        await bot.send_message('<b>⚠️ Что-то пошло не по плану</b>')
